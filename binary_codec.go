...
// TTMLToBinary converts TTML XML text into AMLX binary.
...
// BinaryToTTML converts AMLX binary into TTML XML text.
...
// EncodeBinary encodes a structured lyric into AMLX binary.
func EncodeBinary(ttmlLyric TTMLLyric) ([]byte, error) {
	...
	out.WriteByte(0) // GlobalFlags (v1 unused)
...
// DecodeBinary decodes AMLX binary into a structured lyric.
...
// EncodeAMLX is an alias of EncodeBinary.
func EncodeAMLX(ttmlLyric TTMLLyric) ([]byte, error) {
	return EncodeBinary(ttmlLyric)
}

// DecodeAMLX is an alias of DecodeBinary.
	...
	for lineIndex, line := range lines {
		...
		for wordIndex, word := range line.Words {
			...
			if wordEndMS < wordStartMS {
				// Legacy TTML may have tiny timing inversions; keep the word and clamp duration to 0.
				wordEndMS = wordStartMS
			}

			if wordStartMS < lineStartMS {
				// Legacy TTML may have line begin later than timed words.
